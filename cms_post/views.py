...
def edit_pag(request, name):
    if request.method == "GET":
        try:
            ...
            resp = "La página que has pedido es: "+ page.name + " ==> "+ page.page + "<br>"
            if request.user.is_authenticated():
                resp += "Puedes editar esta página: "+ FORMULARIO
            else:
                resp += "<br>Not logged in, para poderla editar: <a href='/login'>Login</a>"  
        except Pages.DoesNotExist:
			# Cuando no existe
            if request.user.is_authenticated():
                resp = "Esta página no existe, puedes crearla:" + FORMULARIO
            else:
                resp = "Not logged in, para crear la pag: <a href='/login'>Login</a>"                              
    elif request.method == "POST":
        if request.user.is_authenticated():
            try:
                page = request.POST['page']
                pagina = Pages.objects.get(name=name)
                pagina.page = page # Escribo en el contenido de la pag lo del formu
            except Pages.DoesNotExist:
                page = request.POST['page']
